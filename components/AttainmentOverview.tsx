...
export const AttainmentOverview: React.FC<AttainmentOverviewProps> = ({ summary, goals, liabilities }) => {
  ...
  const savingsRate = summary.totalIncome > 0 ? Math.round(((summary.totalIncome - summary.totalExpense) / summary.totalIncome) * 100) : 0;
  ...
  return (
    <Card className="mb-8 border-none bg-gradient-to-r from-slate-900 to-indigo-950 shadow-indigo-500/10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">লক্ষ্যমাত্রা পূরণ</span>
          ...
          <p className="text-[10px] text-slate-500">
            মোট সঞ্চয়: ৳{totalGoalCurrent.toLocaleString()} / ৳{totalGoalTarget.toLocaleString()}
          </p>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">ঋণ মুক্তি</span>
      ...
      <div className="mt-8 pt-6 border-t border-slate-800/50 grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="text-center">
          <p className="text-[10px] text-slate-500 uppercase">মোট দায়</p>
          <p className="text-lg font-bold text-rose-400">
            ৳{(totalDebtTarget - totalDebtPaid).toLocaleString()}
          </p>
        </div>
        <div className="text-center">
